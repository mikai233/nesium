...
/// Spawn a writer task that writes framed bytes to the underlying stream.
///
/// This is used for both TCP and QUIC connections.
///
/// Current behavior:
...
/// Encode a message into a single frame and send it to the outbound queue.
///
...
pub async fn send_msg<T: Message>(tx: &OutboundTx, payload: &T) -> Result<(), ProtoError> {

...
// ================================================================
//  1. Accumulator: LSR A      $4A    1 byte, 2 cycles
// ================================================================
pub const fn lsr_accumulator() -> Instruction {
    ...
    const OP2: MicroOp = MicroOp {
        ...
        micro_fn: |cpu, _| {
            ...
            cpu.a >>= 1; // A = A >> 1 (bit 7 becomes 0)
            cpu.p.reset_n(); // N always reset
...
// ================================================================
//  2. Zero Page: LSR $nn      $46    2 bytes, 5 cycles
// ================================================================
pub const fn lsr_zero_page() -> Instruction {
    const OP1: MicroOp = MicroOp {
        name: "inc_pc",
        micro_fn: |cpu, _| cpu.incr_pc(),
    };
    ...
    const OP5: MicroOp = MicroOp {
        ...
        micro_fn: |cpu, bus| {
            ...
            let result = old >> 1; // shift right, bit 7 = 0
            cpu.p.reset_n(); // N always reset
...
// ================================================================
//  3. Zero Page,X: LSR $nn,X  $56    2 bytes, 6 cycles
// ================================================================
pub const fn lsr_zero_page_x() -> Instruction {
    const OP1: MicroOp = MicroOp {
        name: "inc_pc",
        micro_fn: |cpu, _| cpu.incr_pc(),
    };
...
// ================================================================
//  4. Absolute: LSR $nnnn     $4E    3 bytes, 6 cycles
// ================================================================
pub const fn lsr_absolute() -> Instruction {
    const OP1: MicroOp = MicroOp {
        name: "inc_pc",
        micro_fn: |cpu, _| cpu.incr_pc(),
    };
...
// ================================================================
//  5. Absolute,X: LSR $nnnn,X $5E    3 bytes, 7 cycles
// ================================================================
pub const fn lsr_absolute_x() -> Instruction {
    const OP1: MicroOp = MicroOp {
        name: "inc_pc",
        micro_fn: |cpu, _| cpu.incr_pc(),
    };

...
impl Mapper for Mapper9 {
    ...
    fn ppu_vram_access(&mut self, addr: u16, _ctx: crate::cartridge::mapper::PpuVramAccessContext) {
        // MMC2 could in theory use this hook to keep timing information about
        // when the latches switch, but it does not require CPU-cycle-precise
        // behaviour. Latch updates are driven from `ppu_read` so that the
        // switching tile itself is rendered using the *previous* CHR bank, as
        // documented on Nesdev.
        let _ = addr;
    }

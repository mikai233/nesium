    ...
    pub fn cpu_write(&mut self, addr: u16, value: u8, ppu_bus: &mut PpuBus) {
        ...
        match reg {
            ...
            PpuRegister::Addr => {
                ...
                let observed_addr = committed_v
                    .map(|v| v.raw())
                    .unwrap_or_else(|| self.registers.vram.t.raw());
                ...
                // MMC3-style IRQ counters can clock on A12 rising edges
                // generated by $2006 writes as the VRAM address latch changes.
                ppu_bus.notify_vram_access(observed_addr & ppu_mem::VRAM_MIRROR_MASK, ctx);

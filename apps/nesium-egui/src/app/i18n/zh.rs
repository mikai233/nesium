...
impl LanguagePack for Zh {
    fn text(id: TextId) -> &'static str {
        ...
        match id {
            ...
            InputRecordingSection => "输入录制",
            InputRecordingStatusLabel => "状态:",
            InputRecordingStatusIdle => "空闲",
            InputRecordingStatusRecording => "录制中",
            InputRecordingOutputLabel => "输出文件:",
            InputRecordingEventsLabel => "已捕获事件:",
            InputRecordingStart => "开始录制…",
            InputRecordingStopAndSave => "停止并保存",
            InputRecordingLastSavedLabel => "最近保存:",
            InputRecordingLastErrorLabel => "最近错误:",
            InputRecordingHelp => "按 `frame:pad:state` 写出帧输入事件，可直接用于回放对比。",
